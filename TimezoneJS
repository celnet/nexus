var TimezoneDatabase = (function(){
        ...
        for(var i = 0; i < _tzdb.length; i++){
            _tzdb[i] = _tzdb[i].split(',');
            _tzutcoffset[_tzdb[i][0]] = _tzdb[i][1];
            _tzutcdstoffset[_tzdb[i][0]] = _tzdb[i][2];
        }
            
        return {
            getUTFOffsetByTimezone:function(_timezone){
                var _offset_string = _tzutcoffset[_timezone];
                _offset_sign = _offset_string.substring(0,1);
                _offset_number = _offset_string.substring(1,3) * 3600 + _offset_string.substring(4,6) * 60;
                _offset_number *= 1000;
                return _offset_string == '+'?_offset_number:-_offset_number;
            },
            getUTFDaylightSavingTimeOffsetByTimezone:function(_timezone){
                return _tzutcdstoffset[_timezone];
            }
        };
    })();
